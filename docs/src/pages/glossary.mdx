...
state machine. They can just use plain function calling.
...
If make a workflow without defining any tasks, the steps within the workflow
roll up to a default task.

## Document

A document is an arbitrary document. It can be namespaced to a workflow, or
...
Caching is the method by which we store workflow steps to make them durable and
store raw agent inference interactions to make them durable or power our memory
systems.

## Driver

A driver controls and manages some some lower-level interface, such as access to
...
There is a similar concept, “namespaces”, which Temporal uses in
[the computer science sense](https://en.wikipedia.org/wiki/Namespace).
We aren’t going to expose namespaces explicitly in our API (at least not yet),
but each workspace has its own namespace, and each organization has a default
namespace.

It should be possible to refer to agents or workflows in another workspace if
configured as such. We will likely have a “global” workspace where one can put
“library” agents and such that you want to share across different workspaces.
...
I thought about the term “team”, which is sometimes used, but I think we might
want to use either “team” or “squad” to refer to a collection of agents sometime
in the future. There is also the UNIX concept of “group”, but we’re not going to
use that. At least not right now.
