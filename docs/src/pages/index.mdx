...
Fixpoint's goal is to solve these problems for you so that can focus on the
...
Under the hood, we're working towards supporting every programming language, but
for now we only support Python. You can install Fixpoint with pip:

```bash
pip install fixpoint
```

Then the best way to get started is to look at one of these examples:
...
[lego-brick-mode]: ./lego-brick-mode.md
[library-mode]: ./library-mode.md
[infra-mode]: ./infra-mode.md
