...
class TravelAgent:
    ...
    async def run(self, ctx: WorkflowContext, continent: str) -> None:
...
async def research_cities(ctx: WorkflowContext, continent: str) -> CityList:
    completion = client.chat.completions.create(
        model="gpt-4o",
        messages=[
...
async def plan_itinerary(ctx: WorkflowContext, city: str, country: str) -> None:
