...
If you LLM agents within a **workflow**, then all memories are stored for the
workflow. Future workflow steps can query the memory bank for memories from
earlier in the workflow, and filter by agent, or by the relevant task or step
within the workflow.
