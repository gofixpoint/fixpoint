...
class CloseStatus(Enum):
    """The status of a node when it is closed"""
...
class NodeContents(BaseModel):
    ...
    status: Optional[CloseStatus] = Field(
        description="The status of the node", default=None
    )
...
class CallHandle:
    ...
    def close(self, status: CloseStatus) -> "NodeState":
        """Close the node and return the parent state"""
        # Set status on current node and return parent state
        self._current_state.contents.status = status
        prev_state = self._current_state.prev_state
        if prev_state is None:
            raise ValueError("Cannot close node with no previous state")
...
class NodeState(BaseModel):
    ...
    contents: NodeContents = Field(
        description="The head of the node state", default=NodeContents()
    )
    ...
    def _add_node(self, step: str, task: str) -> "NodeState":
        next_state = NodeState(
            contents=NodeContents(step=step, task=task), prev_state=self
        )
    ...
    def add_step(self, step: str, task: Union[str, None] = None) -> "NodeState":
        ...
        with self._lock:  # pylint: disable=not-context-manager
            task = task or self.contents.task
...
class _SpawnGroup:
    ...
    def __enter__(self) -> "_SpawnGroup":
        return self
    ...
    def __exit__(
        ...
        for node in self._spawned_nodes:
            node.contents.status = CloseStatus.COMPLETED
